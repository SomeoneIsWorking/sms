...
void TMapObjBase::startAnim(u16 param_1)
{
	...
	if (data->unk4) {
		...
		// See [[fileselect-startanim-mtxcalc-null]]: on native, the
		// file-block's MActor can reach here without its model bound (its
		// mModelData->mJointNodePointer[0] chain is null); the GC path
		// derefs unconditionally. Guard the write on native so the
		// "clear current anim" call is a no-op when there is no bound
...
// Empty TMapObjBase virtuals — byte-exact `blr` in the binary; subclasses
// override with concrete behavior. Provided so vtable-referenced base slots
