...
TNameRef* TNameRef::genObject(JSUMemoryInputStream& stream,
                              JSUMemoryInputStream& remainder)
{
	const char* type = getType(stream, remainder);
#ifdef SMS_NATIVE_PLATFORM
	if (type == nullptr) {
		OSPanic(
		    __FILE__, __LINE__,
		    ...
		    (void*)stream.mBuffer, (int)stream.mPosition);
	}
	...
	// child (JDRNameRefPtrList.hpp:29), so the object never enters the tree and
	// a later setup search() returns null -> null deref three frames away (e.g.
	...
	if (obj == nullptr && type != nullptr && type[0] != '\0') {
		...
		static const char* const kUnimplemented[] = {
			...
			// getNameRef_NPC under SB_NPC_ON); these are the non-"NPC"
			// creatures and the manager type names referenced by the Delfino
			// Plaza scene.
			"AnimalBird",
			"AnimalBirdManager",
			"BoardNpcManager",
			...
			// unported NPC/creature manager found by SB_GENOBJ_SKIP_ALL survey
			// on the airport0.arc scene graph. Distinct variant of
			// FruitsBoatManager above.
			"FruitsBoatManagerB",
		};
		bool known = false;
		// All individual NPC actor names (NPC*) are deliberately nulled in
		// getNameRef_NPC while the NPC population is excluded -> tolerate by
		// prefix.
		if (strncmp(type, "NPC", 3) == 0)
			known = true;
		for (unsigned i = 0;
		     !known && i < sizeof(kUnimplemented) / sizeof(*kUnimplemented);
		     ++i) {
		...
		// Survey mode (SB_GENOBJ_SKIP_ALL=1): skip+log EVERY unknown type
		// instead of panicking, so one boot enumerates the whole missing-type
		// set for a new scene (e.g. the plaza) rather than rediscovering them
		// one panic per run. Diagnostic only — leave the default fail-fast on
		// for normal runs.
