...
public:
	...
	JUTResReference()
	{
		...
		// Native diagnostics may inspect the optional name even for an empty
		// reference. Retail only needs the type byte, but carrying an
		// indeterminate length/name onto the host is not a valid representation
		// of that empty state.
		mNameLength = 0;
		mName[0]    = '\0';
#endif
	...
	/* @fabricated: read-only diagnostic accessor (SB_J2D_PIC_DUMP), no behavior
	 * change */
